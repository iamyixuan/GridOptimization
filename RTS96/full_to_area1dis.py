...
def ContGenDispatch(ContNum,scenarioNum):
	...
	with open(file_path, 'r') as f:
		for i, line in enumerate(f):
			newLine = line.strip().replace(' ','').replace("'","").split(',')
			newLine = map(convertingToNum, newLine)
			if i >=2 and i <= 991 and newLine[0] == ContNum and newLine[2] >= 300 and newLine[2] < 400:
...
# dataframe.to_csv('fullInfo_area3.csv')

data = pd.read_csv('fullInfo_area3.csv', header = 0)
data = data.iloc[:,1:]
