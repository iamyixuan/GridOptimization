...
def ContGenDispatch(ContNum,scenarioNum):
	...
	with open(file_path, 'r') as f:
		for i, line in enumerate(f):
			newLine = line.strip().replace(' ','').replace("'","").split(',')
			newLine = map(convertingToNum, newLine)
			if i >=2 and i <= 991 and newLine[0] == ContNum and newLine[2] >= 200 and newLine[2] < 300:
...
# dataframe.to_csv('area2FullData.csv')

data = pd.read_csv('area2FullData.csv', header = 0)
data = data.iloc[:,1:]
